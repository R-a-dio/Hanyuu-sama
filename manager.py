...
class Queue(object):
    ...
    def get_timestamp(type=REGULAR):
        with MySQLCursor() as cur:
            if (type == REGULAR):
                cur.execute(
                    "SELECT unix_timestamp(time) AS timestamp, length FROM `queue` ORDER BY `time` DESC LIMIT 1;")
            elif (type == REQUEST):
                cur.execute(
                    "SELECT unix_timestamp(time) AS timestamp, length FROM `queue` WHERE (type=1 OR type=2) ORDER BY `time` DESC LIMIT 1;")
            if (cur.rowcount > 0):
                result = cur.fetchone()
    ...
    def pop(self):
        ...
        try:
            with MySQLCursor(lock=self._lock) as cur:
                cur.execute(
                    "SELECT * FROM `queue` ORDER BY `time` ASC LIMIT 1;")
                if (cur.rowcount > 0):
                    result = cur.fetchone()
    ...
    def bump(self, song):
        ...
        with MySQLCursor(lock=self._lock) as cur:
            cur.execute(
                "SELECT * FROM `queue` WHERE trackid=%s LIMIT 1;", (song.id,))
            if (cur.rowcount == 1):
                cur.execute(
                    "UPDATE `queue` SET type=1, time=NOW() WHERE trackid=%s LIMIT 1;", (song.id,))
            else:
